...
def getDaily(stationtriplet, element, s_date, e_date, orient):

  server = 'https://api.snowdata.info'
  endpoint='/data/getDaily'
  args = f'?triplet={stationtriplet}&s_date={s_date}&e_date={e_date}&element={element}&format=json&orient={orient}'
  url = f'{server}{endpoint}{args}'
...
def getData(stationparameter, s_date, e_date, orient='records'):   
 
    try:
        data = reduce(lambda left,right: pd.merge(left,right,left_index=True, right_index=True, how='outer'), 
              [getDaily(stationtriplet = i[0], element = i[1], s_date=s_date, e_date=e_date, orient=orient) for i in stationparameter])
...
class RegressionFun():
    ...
    def __init__(self, stationparameterpairs, begindate, enddate, orient='records'): #, stations, parameter_of_interest):
        ...
        self.data = getData(self.stationparameterpairs, begindate, enddate, orient).dropna()
...
#             self.modelfit_fig = fig2
        ...
        if len(self.stations) == 3:
            ...
            fig2.update_layout(
                ...
                scene = dict(
                    xaxis_title = f'{self.stationparameterpairs[0]}',
                    yaxis_title = f'{self.stationparameterpairs[1]}',
                    zaxis_title = f'{self.stationparameterpairs[2]}',
#                     width=700,
...
#                 fig.update_xaxes(title_text = f'{self.stationparameterpairs[i+1]}')
#                 fig.update_yaxes(title_text = f'{self.stationparameterpairs[0]}')
#                 fig.update_layout(
#                     showlegend=True,
#                     height=500,
#                     width=950,
#                     title={
#                       'text': "Regression Model - Slice through axis",
#                       'xanchor': 'center',
#                       'yanchor': 'top',
#                       'y':0.9,
#                       'x':0.4
...
#                 model_slices.append(fig)
                
                
                fig3 = make_subplots(rows=len(self.stations), cols=1, #specs = [[{}, {}]],
                ...
                for i in range(0, len(self.stations)-1):
                    ...
                    fig3.update_xaxes(title_text = f'{self.stationparameterpairs[i+1]}')
                    fig3.update_yaxes(title_text = f'{self.stationparameterpairs[0]}')
                    fig3.update_layout(
                        showlegend=True,
                        height=950,
                        width=950,
                        title={
                          'text': "Regression Model - Slices through axes",
                          'xanchor': 'center',
                          'yanchor': 'top',
#                           'y': .95,
                          'x':0.4
...
#             fig3.update_xaxes(title_text = f'{self.stations[0]} {self.parameters[0]}')
#             fig3.update_yaxes(title_text = f'{self.stationparameterpairs[1:]}')
#             fig3.update_layout(
#                 showlegend=True,
#                 height=500,
#                 width=950,
